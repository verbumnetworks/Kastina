...
async function updateBlogAction(oldSlug: string, formData: FormData) {
  "use server";
...
async function deleteBlogAction(slug: string) {
  "use server";
...
export default async function EditBlogPage({ params }: Props) {
  ...
  const item = await prisma.blog.findUnique({ where: { slug: id } });
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      ...
      <div className="grid gap-6 md:grid-cols-[1fr,16rem]">
        {/* Form */}
        <form
          action={updateBlogAction.bind(null, id)}
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              ...
              <p className="mt-1 text-xs text-slate-500">
                Changing the slug updates the URL (e.g. /Blog/your-slug).
              </p>
            </div>
            <div>
        ...
        <div className="space-y-4">
          ...
          <form action={deleteBlogAction.bind(null, id)}>
            <button
              type="submit"
