...
export default async function AdminHomilyView({ params }: Props) {
  const { slug } = await params; 
  const item = await prisma.homily.findUnique({
    where: { slug },
    select: {
  ...
  return (
    <div className="p-6">
      <div className="mb-4 flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold">Homily Details</h1>
          <p className="text-sm text-slate-500">Slug: <span className="font-mono">{item.slug}</span></p>
        </div>
        <div className="flex gap-2">
          <Link
            href={`/dashboard/homily/${item.slug}/edit`}
      ...
      <div className="grid gap-6 md:grid-cols-[1fr,18rem]">
        ...
        <div className="rounded-2xl border bg-white p-5">
          <div className="flex items-center justify-between text-sm text-slate-500">
            ...
            <span>
              Updated {item.updatedAt.toLocaleDateString("en-US", { year: "numeric", month: "short", day: "numeric" })}
            </span>
          </div>
          ...
          <div className="prose prose-slate mt-6 max-w-none whitespace-pre-wrap">
            {item.content || <p className="text-slate-500">No additional details provided.</p>}
          </div>
        </div>
